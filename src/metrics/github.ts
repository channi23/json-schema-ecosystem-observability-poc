...
export type GithubTopicSearch = {
  total_count: number;
  incomplete_results: boolean;
};

export async function fetchJsonSchemaTopicRepoCount(token?: string): Promise<number> {
  const url = "https://api.github.com/search/repositories";
  const q = "topic:json-schema";

  try {
    const res = await axios.get<GithubTopicSearch>(url, {
      timeout: 10_000,
